...
zokou({ nomCom: "tagall", categorie: 'Group', reaction: "😎" }, async (dest, zk, commandeOptions) => {
  ...
  if (!verifGroupe) { repondre("✋🏿 ✋🏿this command is onlyused in groups 📢"); return; }
  if (!arg || arg === ' ') {
  mess = 'Aucun Message'
  } else {
    mess = arg.join(' ')
...
` ;
 ...
 if (verifAdmin || superUser) {
  ...
  zk.sendMessage(dest, { text: tag, mentions: membresGroupe.map((i) => i.id) }, { quoted: ms })
   ...
   } else { repondre('command verified for admins for only')}
...
zokou({ nomCom: "link", categorie: 'Group', reaction: "🤗" }, async (dest, zk, commandeOptions) => {
  ...
  let mess = `hello ${nomAuteurMessage} , You can copy and share group  link below ${nomGroupe} \n
...
zokou({ nomCom: "promote", categorie: 'Group', reaction: "🤝" }, async (dest, zk, commandeOptions) => {
  ...
  let membresGroupe = verifGroupe ? await infosGroupe.participants : ""
  if (!verifGroupe) { return repondre("Only in groups idiot😡"); }
  ...
  try {
    ...
    if (autAdmin || superUser) {
      if (msgRepondu) {
        ...
        else { return repondre("Only admins can perform these task idiot😡.") }

      } else { repondre("please tag the member to be nominated"); }
    } else { return repondre("idiot you aren't an admin why try these cmds🤦‍♂️ .") }
...
zokou({ nomCom: "apk", reaction: "✨", categorie: "Research" }, async (dest, zk, commandeOptions) => {
  ...
  try {
    ...
    if (searchResults.length === 0) {
      return repondre("*unable to access🤦‍♂️ application, please enter another name*");
    }
